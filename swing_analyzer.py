...
class SwingAnalyzer:
    def __init__(self, velocity_threshold=0.5, contact_angle_min=150,
                 use_adaptive_velocity=False, adaptive_velocity_percent=0.15):
        """
        ...
        self.BACKSWING_THRESHOLD = 0.05  # How far back wrist must go (not currently used)
    ...
    def analyze_swing(self, video_data):
        ...
        if self.USE_ADAPTIVE_VELOCITY:
            ...
            print(f"   Calculated threshold: {velocity_threshold:.4f} ({self.ADAPTIVE_VELOCITY_PERCENT*100:.0f}% of max)")
    ...
    def _detect_phases(self, metrics, frames, velocity_threshold):
        ...
        if phases['forward_swing_start']:
            ...
            for i in range(forward_idx, search_window_end):
                m = metrics[i]
                if (m['elbow_angle'] > self.CONTACT_ANGLE_MIN and
            ...
            if contact_candidates:
                ...
                adjusted_idx = min(len(metrics) - 1, contact_idx + 3)  # Go forward 3 frames
