...
def visualize_swing_phases(
    ...
    while cap.isOpened():
        ...
        if "tracking_quality" in video_data:
            ...
            cv2.rectangle(
                overlay_quality, (width - 350, 10), (width - 10, 130), (0, 0, 0), -1
            )
        ...
        if "BACKSWING" in current_phase or "UNIT TURN" in current_phase:
            ...
            if engine_data.get("hip_shoulder_separation"):
                hip_shoulder_sep = engine_data["hip_shoulder_separation"].get(
                    "max_value", 0
                )
        ...
        if "FINISH" in current_phase:
            ...
            if tempo_data.get("backswing_duration") is not None:
                ...
                cv2.rectangle(
                    overlay_tempo, (10, height - 120), (450, height - 10), (0, 0, 0), -1
                )
    ...
    for phase_name, phase_data in phases.items():
        if phase_data and isinstance(phase_data, dict):
            ...
            if detected:
                ...
                print(
                    f"  ✅ {phase_name}: {timestamp:.2f}s (confidence: {confidence:.2f})"
                )
            else:
    ...
    if engine_data.get("hip_shoulder_separation"):
        print("\nEngine Metrics:")
        print(
            f"  Hip-Shoulder Separation: {engine_data['hip_shoulder_separation'].get('max_value', 0):.1f}°"
        )
        if engine_data.get("max_shoulder_rotation"):
            print(
                f"  Max Shoulder Rotation: {engine_data['max_shoulder_rotation'].get('value', 0):.1f}°"
            )
        if engine_data.get("max_hip_rotation"):
            print(
                f"  Max Hip Rotation: {engine_data['max_hip_rotation'].get('value', 0):.1f}°"
            )
    ...
    if tempo_data.get("backswing_duration") is not None:
        ...
        if tempo_data.get("forward_swing_duration"):
            print(
                f"  Forward Swing Duration: {tempo_data['forward_swing_duration']:.2f}s"
            )
...
def _assign_phases_to_frames(phases, total_frames):
    ...
    forward_swing_frame, forward_swing_conf, forward_swing_reason = get_phase_info(
        "forward_swing"
    )
    contact_frame, contact_conf, contact_reason = get_phase_info("contact")
    follow_through_frame, follow_through_conf, follow_through_reason = get_phase_info(
        "follow_through"
    )
    ...
    for frame_num in range(1, total_frames + 1):
        if unit_turn == 0 and backswing == 0:
            ...
            unit_turn > 0
            and backswing > 0
            and frame_num >= unit_turn
            and frame_num < backswing
        ):
            ...
            and contact > 0
            and frame_num >= forward_swing
            and frame_num < contact
        ):
            frame_phases[frame_num] = (
                "FORWARD SWING",
                forward_swing_conf,
                "Accelerating",
            )
            ...
            contact > 0
            and follow_through > 0
            and frame_num > contact
            and frame_num < follow_through
        ):
...
def _get_phase_color(phase_name, confidence=1.0):
    ...
    if (
        "BACKSWING" in phase_name
        or "UNIT TURN" in phase_name
        or "Ready Position" in phase_name
    ):
        if confidence > 0.8:
...
if __name__ == "__main__":
    ...
    CONTACT_DETECTION_METHOD = (
        "kinematic_chain"  # 'velocity_peak', 'kinematic_chain', or 'hybrid'
    )
